    ...
    def filter_news(self, news_items: List[Dict], top_k: int = 3,
        ...
        if company_name or ticker:
            ...
            focus = (
                ...
                f"[판단 예시 — 반드시 이 종목 기준으로 적용]\n"
                f"  · 이 종목 자신이 신제품·신기술을 세계 최초/먼저 출시하거나 점유율 1위·대형 수주를 달성 → '긍정'(경쟁사가 했을 때와는 정반대).\n"
                f"  · 경쟁사가 신제품·신기술을 먼저 양산/공급하거나 점유율 1위 달성 → 이 종목엔 '부정'(영향 모호하면 '중립'), 절대 '긍정' 아님.\n"
                f"  · 이 종목이 그 품목을 '파는' 입장인데 가격이 오른다(예: 메모리 제조사 + 메모리 가격 인상) → 판매가 상승이라 '긍정'. 반대로 '사는' 입장이면 '부정'.\n"
                f"  · 정치·관세·금리 등 거시 기사가 업계 전반만 언급하고 이 종목 사업과 직접 연결이 불분명 → '제외' 또는 '중립'.\n"
                f"  · 경쟁사 악재(리콜·감산·소송 패소·생산 차질) → 이 종목엔 반사이익으로 '긍정'일 수 있다.\n\n"
            )
        else:
            focus = ""
        prompt = (
