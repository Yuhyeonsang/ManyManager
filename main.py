...
【1】 📰 뉴스 심층 분석
각 뉴스마다 아래 4가지를 작성하세요 (뉴스가 없으면 이 섹션 생략):
  ① 한 줄 핵심 요약 (무슨 일인지)
  ② 이 뉴스가 ETF 구성종목(예: SK하이닉스, 삼성전자 등)에 직접적으로 어떤 영향을 주는지
  ③ ETF 전체 가격에 단기(1~2주)·중기(1~3개월) 영향 예측
  ④ 감성 판정: 🟢 호재 / 🔴 악재 / 🟡 중립 — 그 이유 한 줄
...
【4】 🔭 앞으로의 전망
  ...
  ▸ 중기 전망 (1~3개월): 이 ETF가 추종하는 섹터/테마의 성장성, 글로벌 트렌드와 연결
  ▸ 주요 리스크: 이 ETF 특유의 위험 요인 2~3가지 (예: 특정 종목 쏠림, 반도체 업황 사이클, 환율 등)
  ▸ 주요 기회 요인: 앞으로 이 ETF 가격을 올릴 수 있는 이벤트/트리거 2~3가지

【5】 💡 투자 판단 요약
  - 종합 의견: 지금 이 ETF 어떻게 봐야 하나? (매수 적극 / 분할 매수 검토 / 관망 / 비중 축소 중 택 1 + 이유)
  - 이 ETF의 가장 큰 강점 1가지
  - 가장 조심해야 할 점 1가지
  - 투자 적합 대상: 어떤 성향의 투자자에게 맞는지 (예: "AI·반도체 장기 성장에 베팅하고 싶은 공격적 투자자")
  - ⚡ 한 줄 요약 (20자 이내)
