        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            <ScrollView
              ...
              contentContainerStyle={styles.galleryImgContainer}
              maximumZoomScale={5}
              minimumZoomScale={1}
              showsVerticalScrollIndicator={false}
              showsHorizontalScrollIndicator={false}
  ...
  const renderConditionDetail = () => (
    <View style={styles.condDetail}>
        ...
        <View key={i} style={styles.condDetailRow}>
          <View style={styles.condDetailContent}>
            ...
            <Text style={styles.condDetailMeta}>
              {c.weight_pct != null ? `목표비중 ${c.weight_pct}% (${c.weight_mode === 'add' ? '추가' : '타겟'})` : c.qty != null ? `${c.qty}주` : ''}
              {c.ticker ? ` · ${c.ticker}` : ''}
            </Text>
          </View>
      ...
      {conditions.sell_conditions?.map((c, i) => (
        <View key={i} style={styles.condDetailRow}>
          <View style={styles.condDetailContent}>
            <Text style={styles.condDetailName}>{c.label ? `[${c.label}] ` : ''}{c.name || c.ticker || '-'}</Text>
            <Text style={styles.condDetailCond}>조건: {c.condition}</Text>
            <Text style={styles.condDetailMeta}>
              {c.sell_pct != null ? `${c.sell_pct}% 매도 (${c.sell_mode === 'initial_qty' ? '최초수량 기준' : '현재보유 기준'})` : c.qty === 'all' ? '전량 매도' : c.qty != null ? `${c.qty}주` : ''}
              {c.ticker ? ` · ${c.ticker}` : ''}
            </Text>
          </View>
          <View style={styles.condActions}>
      ...
      {conditions.lock_conditions?.map((c, i) => (
        <View key={i} style={styles.condDetailRow}>
          <View style={styles.condDetailContent}>
            ...
            <Text style={styles.condDetailMeta}>{c.action === 'liquidate' ? '전량 청산 + 매수 잠금' : '신규 매수 잠금'}</Text>
          </View>
          <View style={styles.condActions}>
    ...
    <>
      ...
      {result.trade_log.slice(0, 50).map((t, i) => (
        <View key={i} style={styles.logRow}>
          <Text style={[styles.logAction, t.action === '매수' ? styles.buyText : t.action === '매도' || t.action === '청산' ? styles.sellText : styles.mutedText]}>
            {t.action}
          </Text>
...
const styles = StyleSheet.create({
  ...
  thumbDel: {
    ...
    backgroundColor: '#DC2626', borderRadius: 10, width: 20, height: 20, alignItems: 'center', justifyContent: 'center',
  },
  thumbDelText: { color: '#fff', fontSize: 10, fontWeight: '900' },
  thumbLabel: { fontSize: 10, color: '#6366F1', fontWeight: '600', marginTop: 3 },
  thumbAdd: {
    width: 76, height: 76, borderRadius: 10, borderWidth: 2, borderColor: '#CBD5E1', borderStyle: 'dashed',
  ...
  // 실행 버튼
  runBtn: { marginTop: 20, backgroundColor: '#6366F1', padding: 18, borderRadius: 14, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  runBtnText: { color: '#fff', fontSize: 18, fontWeight: '800' },

  // 결과
  bigCard: { backgroundColor: '#fff', borderRadius: 14, padding: 24, alignItems: 'center', borderWidth: 2, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4, elevation: 2 },
  ...
  metricBox: { width: '48%', backgroundColor: '#fff', borderRadius: 10, padding: 14, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 },
  ...
  stockRow: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 10, padding: 12, marginBottom: 8, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 },
  stockInfo: { flex: 1 },
  ...
  galleryHeader: { position: 'absolute', top: 50, left: 0, right: 0, zIndex: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20 },
  ...
  modalBtns: { flexDirection: 'row', gap: 10, marginTop: 16, paddingBottom: Platform.OS === 'ios' ? 8 : 0 },
